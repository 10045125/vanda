...
public abstract class RecyclingPagerAdapter extends PagerAdapter {
	 ...
	 * position） { try{ 　　　　if（views.get（position）.getParent（）==null）{ 　
	 * container.addView（views.get（position））; 　　　　}else{
	 * （（ViewGroup）views.get（position
	 * ）.getParent（））.removeView（views.get（position））;
	 * container.addView（views.get（position））; } }catch（Exception e）{
	 * e.printStackTrace（）; } return views.get（position）; }
	 */
