...
public class PullToRefreshAttacher implements View.OnTouchListener {
	...
	public PullToRefreshAttacher(Activity activity, Options options) {
		...
		if (mHeaderProgressBar == null) {
			throw new IllegalArgumentException(
					"Must supply valid layout id for progressbar.");
		}

		mHeaderContent = mHeaderView.findViewById(R.id.ptr_content);
		if (mHeaderContent == null) {
			throw new IllegalArgumentException(
					"Must supply valid layout id for prt_content.");
		}
	 ...
	 * @param refreshListener
	 *            - Listener to be invoked when a refresh is started.
	 * @param alterTouchListeners
	 *            - Whether to set this as the touch listener for
	 *            <code>view</code>. You should only provide <code>false</code>
	 *            here if you will propagate the touch events to this object
	 *            yourself.
	 */
	public void setRefreshableView(View view, ViewDelegate viewDelegate,
	...
	private int getScrollNeededForRefresh() {
		return Math
				.round(mRefreshableView.getHeight() * mRefreshScrollDistance);
	}
	...
	public static class DefaultHeaderTransformer extends HeaderTransformer {
		...
		protected Drawable getActionBarBackground(Context context) {
			...
			context.getTheme().resolveAttribute(android.R.attr.actionBarStyle,
					outValue, true);
			// Now get action bar style values...
			TypedArray abStyle = context.getTheme().obtainStyledAttributes(
					outValue.resourceId, android_styleable_ActionBar);
			try {
